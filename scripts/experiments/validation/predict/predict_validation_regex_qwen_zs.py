"""Fill empty regex predictions with Qwen ZS predictions.

Uses regex-based TNM predictions as the primary source and fills empty cells
(where no regex match was found) with predictions from the Qwen ZS model.
"""
...
PREDICT_DIR = SUBMISSION_DATA_DIR / '01-validation' / 'predict'
REGEX_PATH = PREDICT_DIR / 'predictions_regex.csv'
QWEN_ZS_PATH = PREDICT_DIR / 'predictions_qwen_zs.csv'
OUTPUT_PATH = PREDICT_DIR / 'predictions_regex_qwen_zs.csv'


def main():
    regex_df = pd.read_csv(REGEX_PATH)
    qwen_zs_df = pd.read_csv(QWEN_ZS_PATH)

    print(f'Loaded {len(regex_df)} rows from {REGEX_PATH.name}')
    print(f'Loaded {len(qwen_zs_df)} rows from {QWEN_ZS_PATH.name}')
    ...
    for stage in STAGES:
        ...
        qwen_col = (
            qwen_zs_df.set_index('patient_filename')
        ...
        print(
            f'{stage.upper()}: {n_missing} empty cells filled from qwen_zs'
